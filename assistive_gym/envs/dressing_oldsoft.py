...
class DressingEnv(AssistiveEnv):
    ...
    def step(self, action):
        ...
        self.cloth_attachment.set_base_pos_orient(pos + np.array([0, 0.01, 0]), orient)
    ...
    def reset(self):
        ...
        target_ee_pos = np.array([0.41, -0.6, 1.02])
        ...
        self.cloth = p.loadSoftBody(os.path.join(self.directory, 'clothing', 'sleeve_585v.obj'), scale=0.75, mass=0.1, useNeoHookean=0, useBendingSprings=0, useMassSpring=0, useSelfCollision=1, collisionMargin=0.001, frictionCoeff=0.25, useFaceContact=1, physicsClientId=self.id)
        ...
        self.cloth_attachment = self.create_sphere(radius=0.02, mass=0, pos=new_vertex_position, visual=True, collision=False, rgba=[0, 0, 0, 1], maximal_coordinates=True)
